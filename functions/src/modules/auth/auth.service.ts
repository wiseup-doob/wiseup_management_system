...
export async function login(credentials: LoginRequest): Promise<LoginResponse> {
  ...
  const isPasswordValid = await verifyPassword(credentials.password, user.user_password_hash);
...
export async function changePassword(userId: string, data: ChangePasswordRequest): Promise<void> {
  ...
  // 현재 비밀번호 확인
  const isCurrentPasswordValid = await verifyPassword(data.currentPassword, user.user_password_hash);
