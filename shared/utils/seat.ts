 ...
 */
export function generateSeatData(): Seat[] {
  const seats: Seat[] = [];
 ...
 */
export function generateSeatGrid(totalSeats: number = SEAT_CONFIG.TOTAL_SEATS): Seat[] {
  const seats: Seat[] = [];
 ...
 * 좌석 ID로 좌석 찾기
 */
export function findSeatById(seats: Seat[], seatId: string): Seat | undefined {
 ...
 */
export function findSeatByStudentId(_seats: Seat[], _studentId: string): Seat | undefined {
 ...
 * ❌ 이 함수는 더 이상 사용하지 않음 - seat_assignments에서 조회해야 함
 */
export function visualizeSeatGrid(
  _seats: Seat[], 
  rows: number = SEAT_CONFIG.ROWS, 
  cols: number = SEAT_CONFIG.COLS
): string {
 ...
 * ❌ 이 함수는 더 이상 사용하지 않음 - seat_assignments에서 조회해야 함
 */
export function getAssignedSeatCount(_seats: Seat[]): number {
 ...
 * ❌ 이 함수는 더 이상 사용하지 않음 - seat_assignments에서 조회해야 함
 */
export function isSeatAssigned(_seat: Seat): boolean {
