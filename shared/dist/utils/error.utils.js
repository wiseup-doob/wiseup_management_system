...
export class AppError extends Error {
    ...
    timestamp;
    requestId;
    constructor(type, code, message, statusCode = HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR, details, requestId) {
        super(message || ERROR_MESSAGES[code] || '알 수 없는 오류가 발생했습니다.');
        this.type = type;
        this.code = code;
    ...
    static badRequest(code, message, details, requestId) {
        return new AppError(ErrorType.VALIDATION, code, message, HTTP_STATUS_CODES.BAD_REQUEST, details, requestId);
    }
...
export function normalizeError(error, requestId) {
    ...
    // 기본 내부 서버 에러
    return AppError.internal(ERROR_CODES.INTERNAL_SERVER_ERROR, error.message || '알 수 없는 오류가 발생했습니다.', error, requestId);
}
// 에러 로깅 함수
