 ...
 * 출석 기록 생성
 */
export const createAttendanceRecord = (studentId, date, status, options = {}) => {
 ...
 * 출석 기록에서 시간 계산
 */
export const calculateAttendanceHours = (checkInTime, checkOutTime) => {
 ...
 * 조퇴 여부 확인
 */
export const isEarlyLeave = (checkOutTime) => {
 ...
 * 학생 검색 조건 생성
 */
export const buildStudentSearchQuery = (params) => {
    const query = {};
    // 학생 검색 조건 처리
    if (params.name) {
 ...
 * 출석 기록 검색 조건 생성
 */
export const buildAttendanceSearchQuery = (params) => {
    const query = {};
    if (params.studentId) {
 ...
 * 평균 하원 시간 계산
 */
export const calculateAverageCheckOutTime = (records) => {
 ...
 * 페이지네이션 메타데이터 생성
 */
export const createPaginationMeta = (total, page = PAGINATION_CONSTANTS.DEFAULT_PAGE, limit = PAGINATION_CONSTANTS.DEFAULT_LIMIT) => {
...
export const validatePaginationParams = (page, limit) => {
    ...
    const validatedLimit = Math.min(Math.max(PAGINATION_CONSTANTS.MIN_LIMIT, limit || PAGINATION_CONSTANTS.DEFAULT_LIMIT), PAGINATION_CONSTANTS.MAX_LIMIT);
